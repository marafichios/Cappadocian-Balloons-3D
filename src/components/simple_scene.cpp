...
void SimpleScene::RenderMesh2DOverlay(Mesh* mesh, Shader* shader,
    const glm::mat3& modelMatrix2D)
{
    if (!mesh || !shader || !shader->program)
        return;
    ...
    glm::mat4 projection2D = glm::ortho(
        0.f, (float)resolution.x,
        0.f, (float)resolution.y, 
        -1.f, 1.f);                

    glm::mat4 view2D = glm::mat4(1.f);

    glUniformMatrix4fv(shader->loc_view_matrix, 1, GL_FALSE, glm::value_ptr(view2D));
    glUniformMatrix4fv(shader->loc_projection_matrix, 1, GL_FALSE, glm::value_ptr(projection2D));

    glm::mat4 model = glm::mat4(1);
    model[0][0] = modelMatrix2D[0][0];
    model[0][1] = modelMatrix2D[0][1];
    model[1][0] = modelMatrix2D[1][0];
    model[1][1] = modelMatrix2D[1][1];
    model[3][0] = modelMatrix2D[2][0];
    model[3][1] = modelMatrix2D[2][1];
